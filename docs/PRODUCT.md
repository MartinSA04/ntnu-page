...
- **Proof fragment** (a small ruled red-ink collision showing "Del planen med en lenke"): shipped 2026-07-27, **removed 2026-07-30d**. It was a drawing of two invented courses colliding, and a drawing of the product is not evidence that the product works. The landing page answers with the student's own next lecture instead, which is the real thing. This closes the requirement rather than leaving it open: nothing is meant to replace it. (It was also the site's last `.np-ruled` surface, so the primitive went with it: DESIGN.md §4/§8.)
...
- **Conflict engine — lecture-only hard conflicts** (see DR-1; the draft's "øving-group clustering with alternative-avoidance" is **cut as unbuildable** — §12/DR-1). Øving/lab shown as a muted, non-clashing display label. **Shipped**, including the B7 fix (auto-reveal the muted layer when a plan has entries but zero lecture-classified ones, instead of a blank default week). **2026-07-25:** display-level group/parallel selection shipped alongside it (`groups.ts`) — DR-1's detection scope is unchanged (§0 addendum point 1). **2026-07-28:** the classifier's coverage hole is fixed and measured — see DR-1's qualification. B7's auto-reveal is plan-*global* by design and stays that way; what it could not cover (a lecture-less course sharing a plan with an ordinary one, which silently drops out of both the week and the check) is now said in the margin by `lectureLessCourses`, not drawn.
...
- **Plan-aware clash preview *before* add** — **narrowed to `/emne/[code]/` only, 2026-07-30 (owner's call).** It shipped on all three surfaces (`components/site/planClash.ts`, computed lazily on first hover/focus); the two **search** surfaces — `/emner/`'s rows and the add-course modal — have had it **removed entirely**, along with the hover dwell, the generation guard, the invalidate-on-plan-change plumbing and the "Sjekker kollisjon" indicator that covered the wait. Searching is not the moment a plan is judged: a verdict per visible row is a timetable fetch per row per plan course, and the answer arrives while the student is still scanning names. The verb still appears where a student is looking at *one* course (`/emne/[code]/`, unchanged) and where the plan itself is drawn (`/planlegger/`'s grid, which marks real collisions). `planClash.ts` and its tests are untouched — only the two call sites are gone. **Do not reinstate it on a search surface for consistency**; the asymmetry is the decision.
...
- **DR-1 — Conflict engine is lecture-only.** `TimetableEntry` (models.ts:322) has **no `activityCode`** and no group-membership field; keying on `courseCode+day+start+end` *merges* parallel øving groups instead of distinguishing them, and there is no way to enumerate "alternative slots that avoid the clash." Therefore: **MUST = lecture-only hard conflicts**, lectures classified by keyword on `name`/`title`; øving/lab rendered as a muted, non-clashing display label. **The draft's "only hard-flag when no alternative avoids the clash" is deleted** — it is unbuildable and would produce confidently-wrong answers on every lab-heavy course (c2-F1). Fix first, alongside DR-4. *(Narrowed, not reopened, 2026-07-25 — §0 addendum point 1: conflict* detection *stays lecture-only exactly as above; display-level group/parallel selection is a separate, now-shipped concern, `groups.ts`.)* *(Qualified 2026-07-28, "lectures classified by keyword on `name`/`title`" — the classifier read `title` only, and a full-catalog timetable sweep found **35 % of course-terms with teaching had zero lecture-classified rows**, so DR-1's engine was silently vacuous for a third of the catalog. Root cause: departments that publish no finer title than the delivery-format bucket, so `title === name ===` e.g. "Formidling" (TFY4220's whole 2026_HØST). A bare bucket title is now a lecture signal via a **closed list** in `activity.ts`; the general "fall back to the name bucket whenever the title is opaque" rule was built, scored and **rejected** — it promoted programme names and orientation weeks to lectures, wrong side of this rule's own asymmetry, so `name` is still never consulted and `title` always decides. The same pass moved `/samling/i` from the non-lecture list to the lecture one: a *samling* is the gathering a samlingsbasert (remote/part-time) programme calls its students in for, and that gathering is the teaching — reading it as a mere delivery format was backwards and cost those programmes their whole lecture layer. Together these take 33 % → **20 %**. The residual 20 % genuinely has no lectures (Kunstakademiet, the conservatory) and is handled by DR-8 provenance — `grid.ts`'s `lectureLessCourses` margin note — not by guessing. Auto-revealing those courses' blocks per-course was rejected on the same data: BK1151 alone would add 36 blocks including four all-day "examn week" rows and collide every other course against them.)*
