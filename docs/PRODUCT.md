...
4. **The schedule must render multiple simultaneous items properly** —
   ...
   `grid.ts`, side-by-side clusters + "+N til" overflow) with a block
   popover (`popover.ts`). D14's cut of the week-scrubber reads, adjacent to
...
3. **Free-search add & clash-check.** `/emner/` (a *mode*, not a top-level destination — see §4) with filters → **plan-aware row preview before add** ("ville kollidert med TMA4100") → quick-add → the persistent nav's chip/count-link update sitewide (plan strip deleted 2026-07-25, §4).
...
**The plan strip is deleted, corrected 2026-07-25** — superseded by the
persistent nav's studieinfo chip (state at a glance on every page) plus a
count-link ("N emner · X sp → ukeplanen") shown on every page but the two
that already show the plan, replacing the strip's cross-page continuation
affordance (c3-3) without a layout-dependent sitewide banner. Credits shown
are still best-effort, not authoritative; the planner page's own credit
line is.
...
- Merged weekly grid (`.np-frame.np-ruled`, red-ink collisions). **Shipped**; rebuilt 2026-07-25 as the calendar-engine grid (`layout.ts`/`grid.ts` — side-by-side overlap clusters, "+N til" overflow, a block popover) per §0 addendum point 4: overlap is a supported state, not breakage.
...
- Plan strip (sitewide, with cross-page continuation affordance) (c3-3). **Superseded 2026-07-25** — deleted along with the layout-dependent single nav pill (§0 addendum point 6); the persistent nav's studieinfo chip + count-link carry the same affordance on every page now — see §4.
